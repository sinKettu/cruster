use crate::audit::actions::find::args::ExecutableExpressionArgsValues as Avalue;
...
pub(super) fn exec(left: &Avalue, right: &Avalue) -> Avalue {
    ...
    match (left, right) {
        ...
        (Avalue::Integer(_), _) => {
            Avalue::Boolean(left.integer() == right.integer())
        },
        (Avalue::String(_), _) => {
            Avalue::Boolean(left.string() == right.string())
        },
        _ => {
